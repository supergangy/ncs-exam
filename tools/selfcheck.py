...
# ── 1. 걸러낼 표현 24종 ────────────────────────────────────────────
#
# **목록의 임자는 이 코드다.** 검사를 도는 것이 여기이므로 고칠 일이
# 있으면 여기를 고친다. 이 목록이 어떤 관찰에서 나왔는지는
# docs/PLAN.md 부록 A 「④ 지문 감수위원」 절에 있다.
BANNED = [
...
PROVISO_OK_TYPES = {"규정이해", "규정적용", "절차적용", "금액산출", "사례판정",
                    "코드부여", "조건대조(배정)"}
# 「규정적용」은 유형 정리(#46) 때 생긴 이름인데 이 목록에 빠져 있었다. 허용 여부를
# 가르는 것은 유형명이 아니라 **지문이 규정문이냐**다 — 규정을 읽는 문항은
# 이름이 무엇이든 지문에 「다만」 단서가 들어간다 (2026-08-12)
