...
### 세대는 **배포 횟수**로 센다 — 시간이 아니다

`app.js`·`app.css` 를 한 세대 규칙에 맡겼더니 **20분 만에 늙어 사라졌다.**
옮긴 날 두 번 배포했기 때문이다. 옛 워커를 물고 있는 브라우저는 그때까지
한 번도 오지 않았다 — 구하려던 대상이 오기 전에 구명줄을 걷은 셈이다.

**시간으로 지켜야 하는 것은 세대 규칙에 맡기지 말고 `KEEP` 에 날짜와 함께 적는다.**
...
KEEP = {
    "README.md",
    "data/admin.json",
    # ── 옛 바닐라 판의 껍데기 (2026-08-28 루트 이동) ────────────────
    #   **한 세대 규칙으로는 못 지킨다.** 그 규칙은 「배포 횟수」로 세는데,
    #   옮긴 날 두 번 배포하자 20분 만에 늙어 사라졌다. 옛 워커를 물고 있는
    #   브라우저는 아직 한 번도 안 왔는데 말이다.
    #
    #   옛 캐시의 index.html 이 이 둘을 부른다. 없으면 **흰 화면**이다.
    #   지울 때는 배포 횟수가 아니라 **날짜**로 판단한다 —
    #   2026-11 이후, 로그가 없으니 「석 달이면 왔다 갔다」로 본다.
    "app.js",
    "app.css",
}
