...
| 단 | 할 일 | 산출 |
|---|---|---|
| **1** | `tools/export_bank.py` — `rounds/*/content/*.py` 를 훑어 위 스키마로 적재 | `bank.db` |
| **2** | 유형 붙이기 — `cluster.py` 대표 문구를 `type` 에 넣고 문항을 **반자동** 배정 (사람이 확인) | `type` 574행 씨앗 → 정리 후 수십 개 |
| **3** | 키워드 다대다 채우기 | `item_keyword` |
| **4** | `choice.explain` 분해 — 45문항 자동 초안 + 나머지 집필 | (b) 과제 |
| **5** | 웹 읽기 API + 최소 화면 (유형 목록 → 문항 → 채점 → 해설) | 웹 |
| **6** | 앱 — 콘텐츠 DB 번들 + 사용자 DB 로컬 + 오답노트 PDF | 앱 |
