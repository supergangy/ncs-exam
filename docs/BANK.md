...
순간 아직 안 온 요청이 그물로 떨어지므로 **그 둘을 서버에 남겨 두었다.**

처음에는 한 세대 규칙에 맡겼는데 **20분 만에 늙어 사라졌다** — 그 규칙은 「배포
횟수」로 세는데 옮긴 날 두 번 배포했기 때문이다. 옛 워커를 물고 있는 브라우저는
그때까지 한 번도 오지 않았다. **시간으로 지켜야 하는 것은 세대 규칙에 맡기지 말고
`KEEP` 에 날짜와 함께 적는다** (`tools/deploy_web.py`). 2026-11 이후에 지운다.
...
#### 흰 화면 — 옛 껍데기가 사라진 묶음을 부른다

PC 판을 처음 올리자 배포본이 **하얀 화면**이 되었다(2026-08-27). 순서는 이랬다.

1. 되돌아온 브라우저에 옛 워커가 남아 있다. 그 워커는 **캐시가 먼저**여서
   `/next/` 요청에 **옛 `index.html`** 을 내준다
2. 옛 껍데기는 옛 묶음 이름(`desktop-cUZtaq1U.js` …)을 부른다
3. 그 이름들은 재배포에서 지워졌다 → **404 넷** → 아무것도 실행되지 않는다

`sw.js` 는 캐시 목록에서 SKIP 이라 **버전 해시의 재료에도 없었다.** 그래서 워커의
전략을 고쳐도 판이 그대로였고 옛 캐시가 지워지지 않았다. 네 군데를 고쳤다.

| 어디 | 무엇 |
|---|---|
| `make_sw.mjs` fetch | HTML(이동·`/`·`.html`)은 **그물이 먼저**. 캐시는 오프라인일 때만 |
| `make_sw.mjs` fetch | 그물로 나갈 때 **`no-cache`** 로. 아래 참조 |
| `make_sw.mjs` 버전 | 목록·크기에 **생성기 자신의 바이트**를 섞는다 |
| `tools/deploy_next.py` | 직전 배포의 파일을 **한 세대 남긴다** |

**`fetch(req)` 로는 모자랐다.** 그것은 브라우저 HTTP 캐시를 먼저 보고, GitHub Pages
가 HTML 에 `max-age=600` 을 붙인다 — 워커를 그물 먼저로 고쳐도 **10분 동안 옛
껍데기가 되돌아왔다**(실측). `fetch(url, { cache: 'no-cache' })` 로 매번 서버에
물어본다. 안 바뀌었으면 304 라 값이 거의 없다.

생성한 `sw.js` 는 `new Function(src)` 으로 파싱해 본다 — 주석에 backtick 을 넣어
템플릿 문자열을 끊은 적이 있다(그때는 `make_sw.mjs` 자신이 안 읽혔다).

워커를 고쳐도 **이미 옛 워커를 물고 있는 브라우저는 구하지 못한다** — 그 브라우저는
옛 껍데기를 캐시에서 먼저 준다. 옛 이름이 서버에 남아 있어야 한 번 더 뜨고, 그때
새 워커가 자리를 잡는다. 그래서 배포 쪽에서도 한 세대를 남긴다.

남긴 목록은 `next/carry.json` 에 적어 둔다. **이름이 바뀐 배포에서만 늙힌다.**

    rotated = prev_build - new
    hold    = rotated 가 있으면 rotated, 없으면 carried - new

아랫줄이 빠지면 워커만 고치는 배포(이름이 그대로다)에서 구조용 파일이 사라진다 —
실제로 한 번 그랬고 `--restore-from` 으로 되살렸다. 언제나 직전 한 세대만 함께 있다.
