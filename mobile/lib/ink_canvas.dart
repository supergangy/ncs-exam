...
class _InkLayerState extends State<InkLayer> {
  ...
  void _start(Offset local, double pressure) {
    if (widget.settings.mode == InkMode.eraser) {
      _erase(local);
      return;
    }
    setState(() => _live
      ..clear()
      ..add(_toPage(local, pressure)));
  }

  void _move(Offset local, double pressure) {
    if (widget.settings.mode == InkMode.eraser) {
      _erase(local);
      return;
    }
    if (_live.isEmpty) return;
    final p = _toPage(local, pressure);
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(builder: (ctx, box) {
      ...
      return Listener(
        ...
        onPointerDown: (e) => _start(e.localPosition, _pressure(e)),
        onPointerMove: (e) => _move(e.localPosition, _pressure(e)),
        onPointerUp: (_) => _end(),
        ...
        child: SizedBox(
          ...
          child: Row(
            children: [
              ...
              const Spacer(),
              if (settings.mode == InkMode.off)
                Padding(
                  padding: const EdgeInsets.only(right: 8),
                  child: Text('도구를 골라야 그려집니다',
                      style: TextStyle(
                          fontSize: 12,
