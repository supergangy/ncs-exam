...
class ExamsScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final groups = groupRounds(repo.bank.tracks, rounds);
        ...
        builder: (context, _) {
          ...
          return ListView(
            ...
            children: [
              ...
              // 직렬로 묶는다 — 홈의 과목 목록과 같은 규율이다. 한 줄로 세우면
              // 전산직 응시자가 NCS 여섯 개를 헤치고 자기 것을 찾아야 한다
              for (final g in groups) ...[
                SectionTitle(groups.length > 1
                    ? '${g.name} · 회차 ${g.rounds.length}개'
                    : '회차 ${g.rounds.length}개'),
                ...g.rounds.map((r) {
