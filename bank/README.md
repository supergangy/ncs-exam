...
### 전산 150문항 — 과목별 위험도 (2026-08-06 기준)

`python tools/verify_cs.py` — **검증 150건 · 불일치 0 · 미검증 0**
...
`low` 77건은 `verify_cs.py` 가 **매번 다시 계산해** 정답을 확인한다 — SQL 실행,
스케줄링·페이지교체 시뮬레이션, 다익스트라, 모듈로-2 나눗셈, 진리표 전수 탐색.
문항의 정답 번호를 고치면 검증이 즉시 불일치를 낸다.

`high` 25건은 **소프트웨어공학(8)과 정보보안(7)에 몰려 있다.** 두 과목은
계산으로 확정되는 항목 자체가 적다. 반대로 데이터베이스론과 데이터통신은
`high` 가 없다.
